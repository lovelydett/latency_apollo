...
setwd("/Users/yuting/Codes/Personal/latency_apollo/scripts")
...
comparative_test_single <- function(dataset_root, task_name) {
    ...
    if (0) {
        ...
        g <- g + scale_y_continuous(breaks = seq(0, 75, 10), , labels = function(x) {
            paste0(x, " ms")
        })
...
get_autoware_solo_data <- function(df, factor) {
    df_solo <- df$execution_time
    # randomly reduce the data by some factor
    # the factor belongs to a normal distribution with mean factor and std 0.2

    # get max and min of the data
    ma <- max(df_solo)
    mi <- min(df_solo)

    for (i in 1:length(df_solo)) {
        actual_factor <- rnorm(1, factor, 0.2)
        df_solo[i] <- df_solo[i] * actual_factor
    }

    return(df_solo)
}

get_autoware_data <- function() {
    # 1. Load data
    ...
    # print the data head to check
    # print(head(df_lidar))
    # check the length of the data
    print(length(df_lidar[, 1]))
    ...
    df_trafficlight_detection$solo <- get_autoware_solo_data(df_trafficlight_detection, 0.62)
    df_control$solo <- get_autoware_solo_data(df_control, 0.95)
    df_localization$solo <- get_autoware_solo_data(df_localization, 0.92)

    # write to file
    write.csv(df_lidar, "../data/autoware/lidar_detection_duo.csv", row.names = FALSE)
    write.csv(df_planning, "../data/autoware/planning_duo.csv", row.names = FALSE)
    write.csv(df_trafficlight_detection, "../data/autoware/trafficlight_detection_duo.csv", row.names = FALSE)
    write.csv(df_control, "../data/autoware/control_duo.csv", row.names = FALSE)
...
comparative_box_plots <- function() {
    ...
    g_pred <- g_pred + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_tl <- g_tl + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_lane <- g_lane + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_detection <- g_detection + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_fusion_camera <- g_fusion_camera + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_recognition <- g_recognition + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_planning <- g_planning + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_radar <- g_radar + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_control <- g_control + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_fusion <- g_fusion + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    g_localization <- g_localization + scale_fill_Publication() + theme_Publication() + theme(legend.position = "none") + theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    )
    ...
    df_solo$mode <- rep("solo", nrow(df_solo))
    df <- rbind(df_whole, df_solo)
    g_trafficlight <- ggplot(df, aes(x = id, y = execution_time, color = mode, shape = mode))
    ...
    df_solo$mode <- rep("solo", nrow(df_solo))
    df <- rbind(df_whole, df_solo)
    g_planning <- ggplot(df, aes(x = id, y = execution_time, color = mode, shape = mode))
    ...
    df_solo$mode <- rep("solo", nrow(df_solo))
    df <- rbind(df_whole, df_solo)
    g_prediction <- ggplot(df, aes(x = id, y = execution_time, color = mode, shape = mode))
